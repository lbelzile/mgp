...
mcmc.mgp <- function(dat, mthresh, thresh, lambdau = 1, model = c("br", "xstud", "lgm"), coord, start,
  ...
  for (b in 1:B) {
    ...
    if (model %in% c("xstud", "br")) {
      ...
      scale.c <- lscale.lgm(scale.c, shape.c, ldat,
        lscale.mu = lscale.mu, lscale.precis = lscale.hyp.precis.c,
    ...
    if (model %in% c("xstud", "br")) {
      ...
      shape.c <- shape.lgm(
        scale = scale.c, shape = shape.c, ldat = ldat, shape.mu = shape.mu,
    ...
    if (model == "lgm" && !cshape) {
      ...
      shape.hyp.tausq.c <- 1 / rgamma(
        n = 1, shape = shape.fhyp.tausq.a + 0.5 * (D + ncol(Xm)),
    ...
    if (thin == 1 || b %% thin == 0) {
      ...
      if (model %in% c("xstud", "br")) {
        ...
        lpost[i] <- sum(unlist(lapply(1:D, function(j) {
          mev::gpd.ll(par = c(scale.c[j], ifelse(cshape, shape.c[1], shape.c[j])), dat = ldat[[j]])
        })))
      }
      res[i, 1:D] <- scale.c
